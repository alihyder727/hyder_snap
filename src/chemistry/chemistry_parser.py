...
def write_header(fname):
  ...
  name = result.group(1)
  header = '''/** @brief Implement %s Chemistry
 *
...
void ChemistryBase<%s>::AssembleReactionMatrix(Eigen::DenseBase<D1>& rate,
  Eigen::DenseBase<D2>& jac, Real const q[], Real time, int num)
{
  EquationOfState *peos = pmy_block->peos;
  Thermodynamics *pthermo = pmy_block->pthermo;
  Real cv_ratios = new Real [num]
  Real cvt = 0, qd = 1;
  for (int i = 1; i < num; ++i) {
    cv_ratios[i] = pthermo->GetCvRatio(i)*pthermo->GetMassRatio(i);
    cvt += q[i]*cv_ratios[i];
  }
  for (int i = 1; i <= NVAPOR; ++i)
    qd -= q[i];
  cvt = cvt + qd;
  cvt *= Thermodynamics::Rgas/(peos->GetGamma() - 1.);
'''
  header = header % (name, name)
  return header

def write_footer():
  footer = '''  delete [] cv_ratios;
}
'''
  return footer
...
def write_reaction(rmap, vlist, rtype, relations = None):
  ...
  for name,c in istoi:
    ...
    output += rate_line
    if 'enthalpy' in rmap.keys():
      output += 'rate(iT) += (%s)*(%s)/cvt;\n' % (rstr, rmap['enthalpy'])
    for s in syms:
