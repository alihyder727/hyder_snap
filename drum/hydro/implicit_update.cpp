...
void Hydro::ImplicitUpdate(AthenaArray<Real> &du)
{
  ...
  if (implicit_flag == 2) {
    ...
    for (int k = ks; k <= ke; ++k)
      for (int j = js; j <= je; ++j)
        for (int i = is; i <= ie; ++i) {
          du(IDN,k,j,i) = pimp1->du_(IDN,k,j,i) + pimp2->du_(IDN,i,k,j) + pimp3->du_(IDN,j,i,k);
          du(IVX,k,j,i) = pimp1->du_(IVX,k,j,i);
          du(IVY,k,j,i) = pimp2->du_(IVY,k,j,i);
          du(IVZ,k,j,i) = pimp3->du_(IVZ,k,j,i);
          du(IEN,k,j,i) = pimp1->du_(IEN,k,j,i) + pimp2->du_(IEN,i,k,j) + pimp3->du_(IEN,j,i,k);
        }
  }
}
